...
class MainActivity : ComponentActivity() {
    ...
    private fun TimesScreen(
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            TimeView.entries.forEach { timeView ->
                Button(onClick = { selectedTimeView = timeView }) {
                    Text(timeView.label)
                }
            }
        }
        ...
        Text(
            text = "Eintraege ${selectedTimeView.label}",
                    ...
                    Column(modifier = Modifier.padding(12.dp)) {
                        ...
                        Text(
                            "Netto: ${calculation.netMinutes} min | " +
                                "Saldo Block: ${calculation.balanceMinutes} min | " +
    ...
    private fun SummaryCard(
        ...
        Card(modifier = Modifier.fillMaxWidth()) {
            Column(
                ...
                Text(
                    text = "Zusammenfassung ${selectedTimeView.label}",
                    style = MaterialTheme.typography.titleLarge
                )
    ...
    private fun filterEntriesByView(
        entries: List<WorkTimeEntryEntity>,
        timeView: TimeView
        ...
        return entries.filter { entry ->
            val entryDate = Instant.ofEpochMilli(entry.startEpochMillis)
                .atZone(zoneId)
                .toLocalDate()

            when (timeView) {
                TimeView.Day -> entryDate == today
                TimeView.Week -> entryDate.get(weekFields.weekOfWeekBasedYear()) == currentWeek &&
                    entryDate.get(weekFields.weekBasedYear()) == currentWeekYear
                TimeView.Month -> entryDate.year == today.year && entryDate.month == today.month
                TimeView.Year -> entryDate.year == today.year
            }
        }
    }
    ...
    private fun calculateSummary(
        ...
        entries.forEach { entry ->
            val gross = ((entry.endEpochMillis - entry.startEpochMillis) / 60000L)
                .toInt()
                .coerceAtLeast(0)
        ...
        val workdayCount = entries
            .map { entry ->
                Instant.ofEpochMilli(entry.startEpochMillis)
                    .atZone(zoneId)
                    .toLocalDate()
            }
            .distinct()
