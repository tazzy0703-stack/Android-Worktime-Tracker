...
class MainActivity : ComponentActivity() {
    ...
    private fun TimesScreen(modifier: Modifier, dao: WorkTimeDao, entries: List<WorkTimeEntryEntity>, targetMinutesPerDay: Int, onMessage: (String) -> Unit) {
        ...
        editEntry?.let { entry ->
            EditEntryDialog(entry = entry, dao = dao, onDismiss = { editEntry = null }, onMessage = onMessage)
        }
    ...
    private fun SettingsScreen(
        ...
        LazyColumn(modifier = modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            item { GeofenceSettingsCard(dao, currentSettings, radiusText, onRadiusTextChange, fusedLocationClient, geofenceManager, onMessage) }
    ...
    @Composable
    private fun GeofenceSettingsCard(dao: WorkTimeDao, settings: SettingsEntity, radiusText: String, onRadiusTextChange: (String) -> Unit, client: FusedLocationProviderClient, geofenceManager: GeofenceManager, onMessage: (String) -> Unit) {
        Card(modifier = Modifier.fillMaxWidth()) {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    Button(onClick = {
                        ...
                        if (lat == null || lon == null) onMessage("Keine Arbeitsplatz-Koordinate gesetzt") else {
                            try { geofenceManager.registerWorkplaceGeofence(lat, lon, settings.geofenceRadiusMeters); onMessage("Geofence registriert") } catch (ex: SecurityException) { onMessage("Standortberechtigung fehlt: ${ex.message}") }
                        }
                    }) { Text("Geofence registrieren") }
                    Button(onClick = { geofenceManager.unregisterWorkplaceGeofence(); onMessage("Geofence entfernt") }) { Text("Geofence entfernen") }
                }
    ...
    @Composable
    private fun AppResetCard(dao: WorkTimeDao, onMessage: (String) -> Unit) {
                ...
                confirmButton = {
                    TextButton(onClick = {
                        CoroutineScope(Dispatchers.IO).launch {
                            ...
                            runOnUiThread { onMessage("App Reset abgeschlossen"); confirmReset = false }
                        }
                    }) { Text("Reset") }
                },
    ...
    private fun saveMarkerFromMap(dao: WorkTimeDao, settings: SettingsEntity, point: GeoPoint, onMessage: (String) -> Unit) {
        CoroutineScope(Dispatchers.IO).launch { dao.upsertSettings(settings.copy(geofenceLatitude = point.latitude, geofenceLongitude = point.longitude)); runOnUiThread { onMessage("Arbeitsplatz-Marker aus Karte gespeichert") } }
    }
    ...
    private fun saveCurrentLocationAsWorkplace(client: FusedLocationProviderClient, dao: WorkTimeDao, settings: SettingsEntity, radiusText: String, onMessage: (String) -> Unit) {
        ...
        client.lastLocation.addOnSuccessListener { loc ->
            ...
            CoroutineScope(Dispatchers.IO).launch { dao.upsertSettings(settings.copy(geofenceLatitude = loc.latitude, geofenceLongitude = loc.longitude, geofenceRadiusMeters = radius.toFloat())); runOnUiThread { onMessage("Arbeitsplatz-Koordinate gespeichert") } }
